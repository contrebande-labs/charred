...
def setup_dataset(cache_dir, image_column, caption_column, resolution, tokenizer):
    ...
    laion2b_en = load_dataset(
        ...
        cache_dir=os.path.join(cache_dir, "laion2B-en"),
        split="train",
        streaming=True,
    )
    laion2b_multi = load_dataset(
        "laion/laion2b-multi-vit-l-14-embeddings",
        cache_dir=os.path.join(cache_dir, "laion2B-multi"),
        split="train",
        streaming=True,
    )
    laion1b_nolang = load_dataset(
        "laion/laion1b-nolang-vit-l-14-embeddings",
        cache_dir=os.path.join(cache_dir, "laion1B-nolang"),
        split="train",
        streaming=True,
    )
